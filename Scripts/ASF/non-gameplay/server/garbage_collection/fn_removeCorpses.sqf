// http://www.thebutcherbay.org/showthread.php?tid=63
...
// true/false = on/off.
_canMove = false;          	// if vehicles should be deleted when they are too damaged to move and empty, NOTE empty fuel also means it cannot move.
...
_viewDist = 500;            // min distance in meter from a player unit to allow delete, if you dont care if player sees the delete, set it to 0.
...
_abaTimer = 300;          	// x seconds a vehicle must be unmanned to be deleted, for _abandoned option.
